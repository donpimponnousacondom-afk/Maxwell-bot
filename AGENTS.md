...
- Retain full useful diagnostics in the identity-global private last-ten incident history. `!error` requires an explicit index 0–9 (0 newest) and current bot-admin authorization. **Every response/report goes to the requesting admin's one-to-one DM**, including server invocations; no target override, shared-channel report fallback or new public file/API export. Long reports may use complete UTF-8 attachments. Known credentials/auth material are redacted without dropping upstream explanations, paths, traces or received error bodies.
