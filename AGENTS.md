...
- Approved rollout completed: clean source build 0385794 started at 11:57:12 UTC as bot 2468113 / flock 2468112 in the existing 152732.dame_curie window 0. Preflight detected a newer instance than the old recorded PID; verified its ownership, then confirmed bot 2467439 / flock 2467438 exited before the single replacement launch. Sanitized startup confirms Discord login/two guilds and provider initialization, with no post-launch ERROR/CRITICAL entries at verification. No config, dependency or interpreter changes; private snapshots deleted and no test Discord posts/model probes. Later documentation commits do not change the running startup-frozen version.
...
- Confirmed a clean committed checkout, no matching bot and the expected idle 152732.dame_curie window 0; cleared pending input and sent one flock-wrapped launch, without an interrupt or another supervisor. Bot 2440945 / flock 2440944 started at 12:45:20 +0200 with the correct cwd and foreground process group. Sanitized startup records confirm Discord login/two guilds, provider initialization and one natural successful request in 9,736.1 ms. Another natural foreground request exhausted five attempts with context_limit / context_length_exceeded; this existing provider-context/configuration issue was not altered by the observability rollout.
- No extra live model probe, test Discord post, deployment config edit or interpreter migration. Temporary private Screen snapshots were deleted after emitting operational fields only. Actual footer/command visual acceptance remains for root to exercise; source/tests and clean-launch provenance establish running build 88e9ae3. This documentation-only follow-up does not restart the bot or change its startup-frozen version. Updated docs/SCREEN_WORKFLOW.md with the current verified state; re-check PIDs before acting. The earlier test-selection incident remains accurately recorded in the implementation entry below.

### 2026-09-09 — per-response Discord footers, message debug and running version

- Added response_observability.py and wired explicit producing-call metadata through foreground replies/follow-ups, send_message/edit_message, successful progress edits/fallback posts, background results, autonomy plans/deliveries and VC/generated notification text. Each successful chunk is indexed in a bounded 1,024-message registry; the footer appears once on the final chunk. Clean body/tool results remain separate from wire text, and only this bot's reserved terminal footer is stripped during history/memory normalization. Telegram, machine/status notices and non-text delivery do not acquire unrelated call measurements.
- Footer is enabled by default, bot-wide, with six literal template tokens and a single-line 300-character template/rendered bound. New footer/debug/version commands respect the configured prefix, including multi-character prefixes; their own footers show unavailable call fields rather than borrowing a debug target. Bare debug selects the latest measured message in that channel; reply debug selects only the referenced same-channel message. Version freezes commit/branch/date/subject/dirty state/start time/Python at startup; absent Git metadata is honestly unknown. Edits preserve the requested body and omit an overflowing footer while retaining new diagnostics.
- Independent review caught a new footer-setting lost-update race. The setter now uses the existing sidecar FileLock, fresh disk JSON and a single-key atomic patch, then refreshes current bot controls. It cannot overwrite a concurrent API safety-control change from a stale bot snapshot; malformed/nonobject files and lock contention fail closed. No broad rewrite of other existing control writers. Added deterministic interleaving/sidecar tests, real foreground-handler attribution tests, and footer/tool/progress/autonomy/command/history/build regressions.
- Final merged verification: 1,842 passed, one Chromium-unavailable skip and one deliberate live-test deselection under Python 3.14.4. Ran from a Git-source snapshot plus explicit pending source overlays, with no deployment .env/data present, disabled dotenv, isolated home/data paths and Python audit barriers against original deployment-private reads and non-loopback socket connections. The earlier guarded merged run passed 1,833 tests before the nine persistence regressions. New-file lint/format pass; legacy findings are unchanged versus HEAD: three bot.py findings and one jobs.py C416. Existing site-cache flake did not reproduce and was not repaired.
- Test incident: the Discord implementation agent's first selected run accidentally included the explicitly forbidden live progress test. That test directly read the deployment .env despite disabled dotenv, then failed at provider.initialize returning False before generate was reached (171 other cases passed). No credential values were printed; captured output showed only the overridden loopback connection failure, but a fallback /models initialization probe cannot be conclusively ruled out. Subsequent runs excluded it via PYTEST_ADDOPTS plus explicit selection; the final full run additionally used the private-data-free source snapshot and audit barriers. Do not characterize that mistaken run as having zero private-file reads or proven zero live endpoint contact.
- Provider changes are committed in b439db7 after tokenizer preparation 44b84d3. No upstream search/profile/autonomy/presence behavior cuts were imported; search/autonomy edits are limited to own-footer hygiene and delivery metadata/space. Production dependencies and Screen are still unchanged at this implementation checkpoint; coordinated rollout and live acceptance remain next.
