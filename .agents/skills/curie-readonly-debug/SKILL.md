...
- **Machine view:** `sudo -n /usr/local/bin/python3.14 /opt/maxwell/scripts/instance.py curie logs --format jsonl`. This follows logs and includes the initial tail; do not run it bare into agent output. Capture/filter it privately and manage/stop the follower job when finished. Ctrl-C stops the follower, not Curie.
