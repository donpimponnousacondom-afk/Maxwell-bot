...
| Safe backend-change invalidation/backfill | Implemented: tagged vectors/cache, preserved raw facts, explicit-db maintenance | 178 focused isolated Python 3.14 cases pass; independent review regressions repaired | Pending real Ollama write/search/backfill/recovery | Not deployed |
...
- Two private rootless engines, models, APIs and dashboards are live-checked with synthetic configuration. Final source-frozen images, full regression, real RAG semantic acceptance, restore and production cutover remain in progress. Actual validation exposed and repaired a nonexistent Docker CLI tag, an incompatible Pillow pin, an Ollama/model version mismatch and Caddy's incompatible file capability; none was waived as a test-environment problem.
- RAG/doctor fixes are implemented and focused-tested; real endpoint and full merged-suite acceptance remain in progress. The legacy host-native installer is not the new rootless provisioning path.
