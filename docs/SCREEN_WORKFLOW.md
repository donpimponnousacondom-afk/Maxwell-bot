...
```sh
flock -n /tmp/dame-curie-maxwell.lock sudo -n /usr/local/bin/python3.14 /opt/maxwell/scripts/instance.py curie logs
```
...
- To refresh only the console after updating the log filter, stop the verified log follower with Ctrl-c and run `flock -n /tmp/dame-curie-maxwell.lock sudo -n /usr/local/bin/python3.14 /opt/maxwell/scripts/instance.py curie logs` in the same window. This does not run `up`, reconnect the bot or create another Screen session.
