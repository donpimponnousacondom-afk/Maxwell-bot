...
Version-display build `0385794` started on 2026-09-09 at **11:57:12 UTC** as bot `2468113` under flock `2468112`, in the existing `152732.dame_curie` window 0. Root approved the restart. After detecting and verifying the newer running instance, sent one interrupt and confirmed bot `2467439` / flock `2467438` exited; verified the idle shell, cleared input and launched once from a clean committed checkout. Correct cwd/foreground ownership, Discord login/two guilds and provider initialization were verified; no post-launch ERROR/CRITICAL entries appeared at verification. No dependency/config/interpreter changes or extra probes; private Screen snapshots were deleted. The full isolated Python 3.14.4 suite passed 1,846 tests (one browser skip, one live-test deselection). `!version` now uses UTC for both dates and puts the entire report/footer in a code block. Subsequent notes-only commits do not change the running build or require another restart.

### Previous observability rollout
